...
const factSchema = Joi.object({
  ...
  funFact: Joi.string().max(500).optional(),
...
// GET /api/facts - Get all facts with filtering and pagination
...
// GET /api/facts/:id - Get single fact by ID
...
router.get('/random/single', async (req, res, next) => {
  try {
    ...
    const count = await prisma.fact.count({ where });
    if (count === 0) {
      return res.status(404).json({ error: 'No facts found matching criteria' });
    }

    const randomIndex = Math.floor(Math.random() * count);
    const [fact] = await prisma.fact.findMany({
      where,
      include: {
        category: {
          select: { id: true, name: true, color: true, icon: true }
...
router.post('/', async (req, res, next) => {
  try {
    const { error, value } = factSchema.validate(req.body);
    if (error) {
      return res.status(400).json({ 
        error: 'Validation error', 
        details: error.details[0].message 
